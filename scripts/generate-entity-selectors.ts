...
function generateSelectorComponent(info: EntitySearchInfo): string {
  ...
  lines.push(`    {`);
  lines.push(`      searchText: inputValue || undefined,`);
  lines.push(`      maxResults: 10,`);
  lines.push(`    },`);
  lines.push(`    {`);
  lines.push(`      enabled: inputValue.length > 0,`);
  lines.push(`    }`);
  lines.push(`  );`);
  lines.push(``);
  ...
  lines.push(`  );`);
  lines.push(``);
  lines.push(`  const handleInputChange = useCallback((_event: any, newInputValue: string) => {`);
