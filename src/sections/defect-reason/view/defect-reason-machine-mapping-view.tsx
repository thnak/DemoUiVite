...
interface DefectReasonMachineMappingViewProps {
  // Optional props if needed
}

export function DefectReasonMachineMappingView(props: DefectReasonMachineMappingViewProps) {
  const { id } = useParams();
  ...
  // Fetch defect reason data
  const { data: reasonData, isLoading: isLoadingReason } = useGetDefectReasonById(id || '', {
    enabled: !!id,
  });
  ...
  const handleAddSelected = useCallback(async () => {
    ...
    if (!id) {
      setErrorMessage('Defect reason ID is required');
      return;
    }
  ...
  const handleBack = useCallback(() => {
    navigate(`/defect-reasons/${id}/edit`);
  }, [navigate, id]);
      ...
      <Box sx={{ mb: 5 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
          ...
          <Typography variant="h4">
            Machine Mapping - {reasonData?.name || 'Defect Reason'}
          </Typography>
        </Box>
