  ...
  if (filterName) {
    inputData = inputData.filter(
      (product) =>
        ...
        product.category.toLowerCase().indexOf(filterName.toLowerCase()) !== -1
    );
  }

  if (filterStock && filterStock !== 'all') {
    inputData = inputData.filter((product) => product.stockStatus === filterStock);
  }

  if (filterPublish && filterPublish !== 'all') {
    inputData = inputData.filter((product) => product.publish === filterPublish);
  }
